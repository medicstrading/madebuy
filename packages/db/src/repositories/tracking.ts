...
export async function ensureIndexes(): Promise<void> {
  ...
  await db
    .collection(EVENTS_COLLECTION)
    .createIndex({ tenantId: 1, event: 1, timestamp: -1 })
  await db
    .collection(EVENTS_COLLECTION)
    .createIndex({ tenantId: 1, source: 1, timestamp: -1 })
  await db.collection(EVENTS_COLLECTION).createIndex({ sessionId: 1 })

  // Daily aggregation indexes
  await db
    .collection(DAILY_COLLECTION)
    .createIndex({ tenantId: 1, date: -1 }, { unique: true })
}
    ...
    .aggregate([
      ...
      },
      {
        $group: {
          ...
          views: {
            $sum: {
              $cond: [{ $in: ['$event', ['page_view', 'product_view']] }, 1, 0],
            },
          },
          enquiries: {
...
export async function getAnalyticsSummary(
  ...
  return {
    ...
    conversionRate:
      stats.totalViews > 0 ? (stats.enquiries / stats.totalViews) * 100 : 0,
    sources,
...
export async function getDailyBreakdown(
  ...
  return result.map((r) => {
    ...
    for (const s of r.sourceData || []) {
      ...
      if (s.enquiries > 0) {
        enquiriesBySource[s.source] =
          (enquiriesBySource[s.source] || 0) + s.enquiries
      }
      if (s.purchases > 0) {
        purchasesBySource[s.source] =
          (purchasesBySource[s.source] || 0) + s.purchases
      }
    }
