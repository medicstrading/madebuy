...
export function CustomBuilderView({
  ...
  const handleAddPage = (type: PageType) => {
    const newPage = createCustomPage(
      type,
  ...
  const handleTogglePageVisibility = (pageId: string) => {
    onPagesChange(
      pages.map((p) => (p.id === pageId ? { ...p, enabled: !p.enabled } : p)),
    )
  }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Build your website from scratch with full control over pages and
            sections.
          </p>
        </div>
      </div>
          ...
          {selectedPage ? (
            <div className="bg-white rounded-2xl border border-gray-200 p-6">
              ...
              <div className="space-y-2 mb-6">
                {selectedPage.sections.map((section, _index) => (
                  <div
                    key={section.id}
