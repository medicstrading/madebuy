    ...
    return (
      <div className="text-center py-12 text-gray-500">
        ...
        <p className="text-sm mt-1">
          Source attribution will appear as visitors come to your store
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-3">
      {data.map((source) => {
        ...
        const shareOfTotal =
          totalViews > 0 ? ((source.views / totalViews) * 100).toFixed(1) : '0'
        ...
        return (
          <div key={source.source} className="relative">
            ...
            <div className="absolute inset-0 flex items-center px-4">
              <div
                className={`flex h-9 w-9 items-center justify-center rounded-lg ${config.bgColor} shadow-sm`}
              >
              ...
              <div className="ml-3 flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-600">
                  {source.views.toLocaleString()} views · {shareOfTotal}% of
                  traffic
                </p>
              </div>
              <div className="text-right ml-2">
                <p
                  className={`text-sm font-bold ${
                    source.purchaseRate >= 3
                      ? 'text-emerald-600'
                      : source.purchaseRate >= 1
                        ? 'text-amber-600'
                        : 'text-gray-600'
                  }`}
                >
