import { blog, media, publish, tenants } from '@madebuy/db'
...
/**
 * Execute the publish logic for a given publish record.
 * This function can be called from both the API route and the cron job.
 *
 * @param tenantId - The tenant who owns the publish record
 * @param recordId - The publish record ID to execute
 * @returns Result of the publish operation
 */
export async function executePublishRecord(
  tenantId: string,
  recordId: string,
): Promise<ExecutePublishResult> {
  // Get the tenant for social connections
  const tenant = await tenants.getTenantById(tenantId)
  if (!tenant) {
    return { success: false, results: [], error: 'Tenant not found' }
  }

  // Get the publish record
  const publishRecord = await publish.getPublishRecord(tenantId, recordId)
  if (!publishRecord) {
    return { success: false, results: [], error: 'Publish record not found' }
  }

  // Check if already published
  if (publishRecord.status === 'published') {
    return { success: false, results: [], error: 'Post already published' }
  }

  // Get media files if any
  const mediaFiles = await Promise.all(
    publishRecord.mediaIds.map((id) => media.getMedia(tenantId, id)),
  )
  const validMediaFiles = mediaFiles.filter((m) => m !== null)

  // Update status to publishing
  await publish.updatePublishRecord(tenantId, recordId, {
    status: 'publishing',
  })
  ...
  try {
    ...
    if (includeBlog && publishRecord.blogConfig) {
      try {
        // Create blog post
        const blogPost = await blog.createBlogPost(tenantId, {
        ...
        results.push({
          ...
          status: 'success',
          postId: blogPost.id,
          postUrl: `/${tenantId}/blog/${blogPost.slug}`,
    ...
    return { success: false, results: failedResults, error: errorMessage }
  }
}
