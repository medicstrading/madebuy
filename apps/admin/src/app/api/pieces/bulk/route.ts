...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!operation || !pieceIds || !Array.isArray(pieceIds) || pieceIds.length === 0) {
    ...
    switch (operation) {
      case 'updateStatus':
        if (!params?.status || !['draft', 'available', 'reserved', 'sold'].includes(params.status)) {
      ...
      case 'updateStock':
        ...
        const stockValue = params.stock === 'unlimited' ? 'unlimited' : Number(params.stock)
      ...
      case 'updateCategory':
        ...
        result = await bulk.bulkUpdateCategory(tenant.id, pieceIds, params.category)
        break
      ...
      case 'setFeatured':
        if (params?.isFeatured === undefined) {
          return NextResponse.json({ error: 'Missing isFeatured value' }, { status: 400 })
        }
        result = await bulk.bulkSetFeatured(tenant.id, pieceIds, Boolean(params.isFeatured))
        break

      case 'setPublished':
        if (params?.isPublished === undefined) {
          return NextResponse.json({ error: 'Missing isPublished value' }, { status: 400 })
        }
        result = await bulk.bulkSetPublished(tenant.id, pieceIds, Boolean(params.isPublished))
        break
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (pieceIds.length === 0) {
      return NextResponse.json({ error: 'No piece IDs provided' }, { status: 400 })
    }
