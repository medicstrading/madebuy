...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (!['material', 'piece'].includes(data.itemType)) {
      return NextResponse.json({ error: 'itemType must be "material" or "piece"' }, { status: 400 })
    }

    const item = await reconciliations.addItem(tenant.id, reconciliationId, data)
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes('Cannot add') || error.message.includes('already added')) {
