...
export default async function MaterialsReportPage() {
  ...
  const materialsByCategory = allMaterials.reduce(
    (acc, material) => {
      ...
      acc[category].totalValue +=
        material.quantityInStock * material.costPerUnit
      return acc
    },
    ...
    <div>
      <div className="mb-6">
        ...
        <p className="mt-2 text-gray-600">
          Analytics and insights about your material costs and usage
        </p>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4 mb-8">
        ...
        <StatCard
          ...
          color={
            averageMargin > 50 ? 'green' : averageMargin > 30 ? 'yellow' : 'red'
          }
        />
        <StatCard
              ...
              .map(([category, data]) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 text-sm text-gray-600">
                    <span>
                      {data.materials.length} material
                      {data.materials.length !== 1 ? 's' : ''}
                    </span>
                    <span>•</span>
                    <span>
                      {data.materials.filter((m) => m.isLowStock).length} low
                      stock
                    </span>
                  </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="text-gray-600">
                            {material.quantityInStock.toFixed(2)}{' '}
                            {material.unit} • {formatCurrency(value)}
                          </div>
                        </div>
                      )
                    })}
            ...
            {(
              await Promise.all(
                allPieces.map(async (piece) => {
                  const cogs = await materials.calculatePieceCOGS(
                    tenant.id,
                    piece.id,
                  )
                  const margin =
                    piece.price && cogs > 0 ? piece.price - cogs : null
                  const marginPercent =
                    piece.price && piece.price > 0 && margin !== null
                      ...
                      {piece.marginPercent !== null && (
                        <div
                          className={`text-sm font-medium ${
                            piece.marginPercent > 50
                              ? 'text-green-600'
                              : piece.marginPercent > 30
                                ? 'text-yellow-600'
                                : 'text-red-600'
                          }`}
                        >
