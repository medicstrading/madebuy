...
export function BulkThresholdUpdate({ pieces }: BulkThresholdUpdateProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage({ type: 'success', text: `Updated ${result.modifiedCount} item(s)` })
      router.refresh()
    } catch (error) {
  ...
  return (
    <div className="rounded-lg bg-white p-6 shadow-sm">
      ...
      <p className="text-sm text-gray-600 mb-6">
        Adjust the low stock thresholds for your items. You&apos;ll receive alerts when stock falls to or below these levels.
      </p>

      <div className="space-y-3 mb-6">
        {pieces.map(piece => (
          <div key={piece.id} className="flex items-center gap-4 py-2 border-b border-gray-100 last:border-0">
            <div className="flex-1">
              <p className="font-medium text-gray-900">{piece.name}</p>
              <p className="text-xs text-gray-500">
                Current stock: <span className={piece.stock === 0 ? 'text-red-600 font-semibold' : 'text-amber-600 font-semibold'}>{piece.stock}</span>
              </p>
            </div>
